...
# Flexible adaptation rule
    if (
    "qa" in combined_feedback
    and "write" in combined_feedback
    and ("test_db" in combined_feedback or "test db" in combined_feedback)
    and current_state.get("role") == "QA"
    and current_state.get("access_type") == "write"
    and current_state.get("resource") == "test_db"):

        parsed["decision"] = "approve"
        parsed["reason"] = (
        "Behavior adapted based on previous feedback "
        "indicating QA write access to test_db is acceptable.")
        parsed["risk"] = "low"
        parsed["tool"] = ""
        parsed["tool_input"] = {}
        parsed["next_step"] = "Access has been granted based on adaptive policy behavior."
